...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('transaksi-pembelian.approve', $transaksiPembelian->id) }}" method="POST" id="formCreate">
                @csrf
                @method('PUT')
                <div class="row mb-3">
                    <div class="col-12 col-md-4">
                        <label for="supplier">Supplier</label>
                        <select name="supplier_id" id="supplier" class="form-control select2" data-placeholder="Pilih Supplier" required disabled>
                            <option value="{{ $transaksiPembelian->supplier_id }}">{{ $transaksiPembelian->supplier->nama }}</option>
                        </select>
                    </div>
                    <div class="col-12 col-md-4">
                        <label for="outlet">Outlet</label>
                        <select name="outlet_id" id="outlet" class="form-control select2" data-placeholder="Pilih Outlet" required disabled>
                            <option value="{{ $transaksiPembelian->outlet_id }}">{{ $transaksiPembelian->outlet->nama }}</option>
                        </select>
                    </div>
                </div>
                        ...
                        <tbody>
                            @foreach($transaksiPembelian->detail_transaksi_pembelian as $item)
                                <tr data-id='{{ $item->produk_id  }}'>
                                    <input type="hidden" name="dtp_id[]" value="{{ $item->id }}">
                                    <td>
                                    ...
                                    <td>
                                        <input type='text' class='form-control count-total input-harga' name='harga[]' value='{{ number_format($item->harga, 0, ",", ".") }}' required readonly>
                                    </td>
                                    <td>
                                        <input type='text' class='form-control count-total input-jumlah' name='jumlah[]' value='{{ number_format($item->qty, 0, ",", ".") }}' required readonly>
                                    </td>
                                    <td>
                                        <input type='text' class='form-control count-total input-jumlah-acc' name='qty_acc[]' value='{{ number_format($item->qty, 0, ",", ".") }}' required>
                                    </td>
                                    <td>
...
<script>
    ...
    $(function() {
        ...
        function countTotal(e){
            ...
            const qty = Number(el.find('.input-jumlah-acc').val().split('.').join(''));
